...
function validatePiUrl(urlString: string): { valid: boolean; error?: string } {
  try {
...
serve(async (req) => {
  ...
  try {
    ...
    // SSRF Protection: Validate pi_url before making any requests
    const urlValidation = validatePiUrl(pi_url);
    if (!urlValidation.valid) {
      console.error(`SSRF blocked: ${urlValidation.error} - URL: ${pi_url}`);
      return new Response(
        JSON.stringify({ error: `Invalid Pi URL: ${urlValidation.error}` }),
